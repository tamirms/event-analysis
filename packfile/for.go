...
// FOR (Frame-of-Reference) integer encoding for groups of uint32 values.
// Values are encoded as bit-packed residuals relative to the group minimum,
// requiring only ceil(log2(max-min)) bits per value.
//
...
func decodeGroup(buf []byte, n int, dst []uint32) (values []uint32, consumed int, err error) {
	if n <= 0 {
		return dst, 0, fmt.Errorf("packfile: FOR decode n must be > 0, got %d", n)
	}
	if len(buf) < 5 {
		return dst, 0, fmt.Errorf("packfile: FOR decode buf too short (%d bytes, need >= 5)", len(buf))
	}
	width := uint64(buf[len(buf)-5])
	if width > 32 {
		return dst, 0, fmt.Errorf("packfile: invalid FOR width %d (max 32)", width)
	}
	groupMin := binary.LittleEndian.Uint32(buf[len(buf)-4:])
	packSize := (int(width)*n + 7) / 8
	consumed = packSize + 5
	if len(buf) < consumed {
		return dst, 0, fmt.Errorf("packfile: FOR decode buf too short for payload (%d bytes, need >= %d)", len(buf), consumed)
	}
	dst = ensureCapU32(dst, n)
	forUnpackResiduals(buf[len(buf)-5-packSize:len(buf)-5], n, width, groupMin, dst)
	return dst, consumed, nil
}

// forRangeWidth computes the minimum value and the bit width needed to
// FOR-encode values. Width is clamped to at least 1.
func forRangeWidth(values []uint32) (minVal uint32, width uint8) {
	minVal = values[0]
	maxVal := values[0]
...
// forPackResiduals bit-packs (values[i] - minVal) into buf starting at the given
// byte offset. buf must have 7 bytes of overshoot past the packed payload for
// safe 8-byte writes.
func forPackResiduals(buf []byte, offset int, values []uint32, minVal uint32, width uint8) {
...
// forUnpackResiduals unpacks n bit-packed residuals from packed and adds groupMin.
...
func forUnpackResiduals(packed []byte, n int, w uint64, groupMin uint32, values []uint32) {
	if w == 0 {
		for i := range values {
			values[i] = groupMin
		}
