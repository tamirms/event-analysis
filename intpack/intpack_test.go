...
func TestEncodeGroupLayout(t *testing.T) {
	...
	gotMin := binary.LittleEndian.Uint32(encoded[len(encoded)-4:])
	if gotMin != wantMin {
...
func TestDecodeGroupWidth0(t *testing.T) {
	...
	decoded, consumed, err := intpack.DecodeGroup(buf[:], 3, nil)
	if err != nil {
		t.Fatalf("DecodeGroup width=0: %v", err)
	}
	if consumed != 5 {
	...
	for i, v := range decoded {
		if v != want[i] {
			t.Errorf("value[%d] = %d, want %d", i, v, want[i])
		}
	}
}
